...
for seed in seeds:
    ...
    """"""
    lab = 'CCLE_drug_out'
    txt_path = 'tuning_CCLE_drug_out_4.txt'
    data_path = 'CCLE_ActArea_drug_out_seed={}.pkl'.format(seed)

    model_ls = ['tCNNS',
                'Precily',
                'DeepDSC']

    dc1 = joblib.load('GDSC_CNV.pkl')
    dc2 = joblib.load('GDSC_MUT.pkl')
    dc_a = dict()
    for each in dc1:
        if type(dc1[each]) == list and type(dc2[each]) == list:
            dc_a[each] = dc1[each] + dc2[each]
        else:
            print('error')

    dc = joblib.load('VAE_5000_dict.pkl')
    dc_b = dict()
    for each in dc:
        if type(dc[each]) == list:
            dc_b[each] = dc[each][:500]
        else:
            print('error')
    ...
    drug_ft_ls = ['SMILES',
                  joblib.load('SMILESVec_dict.pkl'),
                  'ECFP']

    cell_encoder_ls = [CellEncoder.CNN(in_dim=6163 * 2, batch_norm=False),
                       CellEncoder.NULL(),
                       CellEncoder.NULL()]

    drug_encoder_ls = [DrugEncoder.CNN(embedding=False, batch_norm=False),
                       DrugEncoder.NULL(),
                       DrugEncoder.NULL()]

    fusion_module_ls = [FusionModule.DNN(cell_dim=1440, drug_dim=300, dropout=0.5, hid_dim_ls=[1024, 1024, 1024]),
                        FusionModule.DNN(cell_dim=1329, drug_dim=100),
                        FusionModule.DNN(cell_dim=500, drug_dim=256, dropout=0.1, hid_dim_ls=[1000, 800, 500, 100])]
    """"""
    ...
    for i in range(len(model_ls)):
        ...
        train_loader = Data.DrDataLoader(Data.DrDataset(train_data, nBits=256), batch_size=batch_size, shuffle=True)
        val_loader = Data.DrDataLoader(Data.DrDataset(val_data, nBits=256), batch_size=batch_size, shuffle=False)
        test_loader = Data.DrDataLoader(Data.DrDataset(test_data, nBits=256), batch_size=batch_size, shuffle=False)
