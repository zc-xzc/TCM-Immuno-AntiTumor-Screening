...
for seed in seeds:
    ...
    """"""
    lab = 'CCLE_drug_out'
    txt_path = 'tuning_CCLE_drug_out_2_0.txt'
    data_path = 'CCLE_ActArea_drug_out_seed={}.pkl'.format(seed)

    model_ls = ['DAE(EXP_ALL)_CNN(SMILES)_DNN',
                'DAE(EXP_ALL)_AFP(Graph)_DNN']

    cell_ft_ls = [joblib.load('GDSC_EXP_ALL.pkl') for _ in range(len(model_ls))]

    drug_ft_ls = ['SMILES',
                  'Graph']

    cell_encoder_ls = [CellEncoder.DNN(in_dim=17420, hid_dim=512, num_layers=3, ft_dim=100) for _ in range(len(model_ls))]
    for i in range(len(cell_encoder_ls)):
        cell_encoder_ls[i].load_state_dict(torch.load('DAE_ALL.pt'))

    drug_encoder_ls = [DrugEncoder.CNN(flatten=False),
                       DrugEncoder.AttentiveFP(ft_dim=64)]

    fusion_module_ls = [FusionModule.DNN(cell_dim=100, drug_dim=60),
                        FusionModule.DNN(cell_dim=100, drug_dim=64)]
    """"""
