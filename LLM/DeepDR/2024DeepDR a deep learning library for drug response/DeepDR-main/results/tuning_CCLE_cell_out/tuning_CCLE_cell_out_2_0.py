...
for seed in seeds:
    ...
    """"""
    lab = 'CCLE_cell_out'
    txt_path = 'tuning_CCLE_cell_out_2_0.txt'
    data_path = 'CCLE_ActArea_cell_out_seed={}.pkl'.format(seed)

    model_ls = ['DAE(EXP)_CNN(SMILES)_DNN',
                'DAE(EXP)_AFP(Graph)_DNN']

    cell_ft_ls = ['EXP' for _ in range(len(model_ls))]

    drug_ft_ls = ['SMILES',
                  'Graph']

    cell_encoder_ls = [CellEncoder.DNN(in_dim=6163, ft_dim=100) for _ in range(len(model_ls))]
    for i in range(len(cell_encoder_ls)):
        cell_encoder_ls[i].load_state_dict(torch.load('DAE.pt'))

    drug_encoder_ls = [DrugEncoder.CNN(),
                       DrugEncoder.AttentiveFP(ft_dim=64)]

    fusion_module_ls = [FusionModule.DNN(cell_dim=100, drug_dim=300),
