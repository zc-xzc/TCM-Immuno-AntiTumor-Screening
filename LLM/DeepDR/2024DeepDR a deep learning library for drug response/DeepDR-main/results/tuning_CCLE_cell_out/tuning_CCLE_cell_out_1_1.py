...
for seed in seeds:
    ...
    """"""
    lab = 'CCLE_cell_out'
    txt_path = 'tuning_CCLE_cell_out_1_1.txt'
    data_path = 'CCLE_ActArea_cell_out_seed={}.pkl'.format(seed)

    model_ls = ['DAE(EXP)_DNN(ECFP)_DNN']

    cell_ft_ls = ['EXP']

    drug_ft_ls = ['ECFP' for _ in range(len(model_ls))]

    cell_encoder_ls = [CellEncoder.DNN(in_dim=6163, ft_dim=100)]
    cell_encoder_ls[0].load_state_dict(torch.load('DAE.pt'))
