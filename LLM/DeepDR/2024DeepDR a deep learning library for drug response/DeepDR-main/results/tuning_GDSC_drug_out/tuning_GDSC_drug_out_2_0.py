...
for seed in seeds:
    ...
    """"""
    lab = 'GDSC_drug_out'
    txt_path = 'tuning_GDSC_drug_out_2_0.txt'
    data_path = 'GDSC2_IC50_drug_out_seed={}.pkl'.format(seed)

    model_ls = ['DNN(EXP_ALL)_CNN(SMILES)_DNN',
                'DNN(EXP_ALL)_AFP(Graph)_DNN']
    ...
    cell_encoder_ls = [CellEncoder.DNN(in_dim=17420, ft_dim=100) for _ in range(len(model_ls))]
