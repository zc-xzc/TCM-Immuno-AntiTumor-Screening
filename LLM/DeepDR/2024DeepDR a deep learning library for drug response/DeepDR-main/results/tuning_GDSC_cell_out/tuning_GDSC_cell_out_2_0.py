...
for seed in seeds:
    ...
    """"""
    lab = 'GDSC_cell_out'
    txt_path = 'tuning_GDSC_cell_out_2_0.txt'
    data_path = 'GDSC2_IC50_cell_out_seed={}.pkl'.format(seed)

    model_ls = ['CNN(EXP)_CNN(SMILES)_DNN',
                'CNN(EXP)_AFP(Graph)_DNN']
    ...
    cell_encoder_ls = [CellEncoder.CNN(in_dim=6163) for _ in range(len(model_ls))]

    drug_encoder_ls = [DrugEncoder.CNN(),
                       DrugEncoder.AttentiveFP(ft_dim=64)]

    fusion_module_ls = [FusionModule.DNN(cell_dim=1440, drug_dim=300),
                        FusionModule.DNN(cell_dim=1440, drug_dim=64)]
    """"""
